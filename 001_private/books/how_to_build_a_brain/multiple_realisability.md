#lead/computationalphilosophy #core/artificialintelligence #fundamental/logic
...
- **Aristotle's [hylomorphism](../../_general/hylomorphism.md) (4th c. BCE).** [Hylomorphism](../../_general/hylomorphism.md) treats every substance as a union of matter (_hyle_) and form (_morphe_), with the soul as the form of the living body. This already separates _what_ something does from _what_ it is made of, making it the conceptual ancestor of multiple realisability.
- **Putnam (1967), "The Nature of Mental States".** Argued that pain in a human, an octopus, and a hypothetical Martian could be the same mental kind despite radically different neural realisers — refuting type-identity theory (the claim that every mental type is identical to a specific physical type).
- **Fodor (1974), "Special Sciences".** Generalised the argument: higher-level sciences (psychology, economics, biology) carve nature at functional joints that are heterogeneous at the physical level, so their generalisations cannot be reduced to physics without loss.
- **Lewis & Armstrong**: developed compatible analytic-functionalist accounts that treat mental states as occupants of causal roles, leaving the physical realiser an empirical question.

## Conceptual Anatomy

- **Type vs token.** A _type_ is a kind (pain, belief-that-p); a _token_ is a particular occurrence in a specific system at a specific time. Multiple realisability is the claim that one type has many possible token realisations.
- **Realisation relation.** A physical state $P$ _realises_ a mental state $M$ when $P$ instantiates the functional/causal role definitive of $M$. This is stronger than mere [supervenience](../taxonomy_and_metaphysics_of_mind-uploading/supervenience.md): [supervenience](../taxonomy_and_metaphysics_of_mind-uploading/supervenience.md) requires only co-variation, whereas realisation specifies that $P$ implements $M$ in virtue of its functional organisation.
...
A useful refinement is to distinguish _what_ is multiply realisable:
...
- [Hylomorphism](../../_general/hylomorphism.md) — Aristotelian ancestor
- [Functionalism](multiple_realisability_and_functionalism.md) — paired theory of mental kinds
