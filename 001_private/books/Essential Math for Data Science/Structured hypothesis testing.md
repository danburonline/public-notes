...
3. **Collect [Data](../../../004_subsidiary/courses/DataCamp%20ML%20Scientist/Types%20of%20Data.md)**: Collect relevant data through experiments, surveys, or observational studies.
4. **Perform a [Statistical](Common%20statistical%20issues.md) Test**: Conduct a statistical test to determine the [P-value](P-value.md), which is the probability of obtaining the observed data (or more extreme) if the null hypothesis is true.
5. **Make a Decision**: If the [p-value](P-value.md) is less than the significance level, reject the null hypothesis. If not, do not reject the null hypothesis.
6. **Report the Results**: Disclose the results, including the observed data, the statistical test used, the [p-value](P-value.md), and the conclusion.
