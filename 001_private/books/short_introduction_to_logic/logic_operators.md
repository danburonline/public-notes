...
Logical operators are **symbols that connect or modify propositions to form compound statements, whose truth values are determined systematically from the truth values of their components** — the complete behaviour of each operator is tabulated in [truth tables](truth_tables.md).
...
- **[OR (∨)](disjunction.md)**: This is also a binary operator. p OR q (p ∨ q) is true if either p is true, q is true, or both p and q are true.
- **[IF…THEN (→)](conditional_reasoning.md)**: This is a binary operator. IF p THEN q (p → q) is true unless p is true and q is false. It is also known as 'implies' or 'conditional'.
...
- Every operator above assumes exactly two truth values. [Fuzzy logic](fuzzy_logic.md) generalises the classical connectives to degrees of truth, and the [sorites paradox](sorites_paradox.md) illustrates where binary valuation strains under vagueness in natural language.
